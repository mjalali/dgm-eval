...
def cov_rff2_joint(x, feature_dim, std, batchsize=16, presign_omeaga=None, normalise=True):
    ...
    batched_rff = torch.cat([batched_rff_cos, batched_rff_sin, batched_rff_cos_negative, batched_rff_sin_negative],
                            dim=1) / (np.sqrt(2) * np.sqrt(feature_dim))  # [B, 2 * feature_dim]
    ...
    for batchidx in tqdm(range(batch_num)):
        batched_rff_slice = batched_rff[
            batchidx * batchsize:min((batchidx + 1) * batchsize, batched_rff.shape[0])]  # [mini_B, 2 * feature_dim, 1]
        cov += torch.bmm(batched_rff_slice, batched_rff_slice.transpose(1, 2)).sum(dim=0)
        i += batched_rff_slice.shape[0]
    cov /= x.shape[0]
    assert i == x.shape[0]
...
def cov_rff2_joint_v2(x, feature_dim, std, batchsize=16, presign_omeaga=None, normalise=True):
    ...
    batched_rff = torch.cat([torch.cos(product_img) * torch.cos(product_txt),
                             ...
                             torch.sin(product_img) * torch.sin(product_txt)], dim=1) / np.sqrt(
        feature_dim)  # [B, 4 * feature_dim]
...
def joint_random_projection(phi_x, phi_y, feature_dim, batchsize=16, normalise=True, u_x=None, u_y=None):
    if u_x or u_y is None:
        ...
        u_x = (1 / torch.sqrt(torch.tensor(feature_dim))) * torch.empty((feature_dim, phi_x.shape[0]),
                                                                        device=phi_x.device).uniform_(-sqrt_3, sqrt_3)
        u_y = (1 / torch.sqrt(torch.tensor(feature_dim))) * torch.empty((feature_dim, phi_y.shape[0]),
                                                                        device=phi_y.device).uniform_(-sqrt_3, sqrt_3)
...
class ConditionalEvaluation:
    ...
    def gaussian_kernel(self, x, y=None, sigma=None, batchsize=8, normalize=True):
        '''
    ...
    def compute_entropy(self, X, order, compute_kernel=True, algorithm='kernel', omegas=None, sigma=None,
            ...
            if order == 2:
                ...
                if computation_method == 'vendi':
                    ...
                    if algorithm != 'kernel':
                        ...
                        logger.info(
                            f'Using truncated Vendi method for algorithm {algorithm} with top {min(X.shape[0], top_k)} eigenvalues, sum of remaining eigvals: {1 - vals.sum()}')
        ...
        if algorithm == 'nystroem':
            ...
            if sigma == self.sigma[
                0]:  # Image kernel,   TODO Change this code, it will be buggy when sigma image and text are the same
                X = self.nystroem_k_x
    ...
    def compute_joint_entropy(self, X, Y, order, compute_kernel=False, algorithm='kernel', feature_dim=3000,
                              kernel='gaussian', top_k=None, computation_method='vendi'):
        if algorithm == 'kernel':
            # assert X.shape[0] == X.shape[1]
            if kernel == 'gaussian':
        ...
        if algorithm == 'rff':
            ...
            return self.compute_entropy(cov_joint, order, compute_kernel=False, feature_dim=feature_dim, top_k=top_k,
        ...
        if algorithm == 'random_projection':
            ...
            cov_joint, self.u_x, self.u_y, _ = joint_random_projection(phi_x.T, phi_y.T, feature_dim, batchsize=16,
                                                                       normalise=True, u_x=None, u_y=None)
            return self.compute_entropy(cov_joint / torch.trace(cov_joint), order, compute_kernel=False,
                                        computation_method='vendi', feature_dim=feature_dim)
    ...
    def conditional_entropy(self, X, Y, order, n_samples=10_000, compute_kernel=False, algorithm='kernel',
                            feature_dim=3000, kernel='gaussian', top_k=None, computation_method='vendi'):  # H_a(X|Y)
        entropy_joint = self.compute_joint_entropy(X, Y, order=order, compute_kernel=False, algorithm=algorithm,
                                                   feature_dim=feature_dim, kernel=kernel, top_k=top_k,
                                                   computation_method=computation_method)
        entropy_Y = self.compute_entropy(Y, order=order, compute_kernel=True, algorithm=algorithm, omegas=self.omegas_y,
                                         feature_dim=feature_dim, sigma=self.sigma[1], kernel=kernel, top_k=top_k,
                                         computation_method=computation_method)
        entropy_X = self.compute_entropy(X, order=order, compute_kernel=True, algorithm=algorithm, omegas=self.omegas_x,
                                         feature_dim=feature_dim, sigma=self.sigma[0], kernel=kernel, top_k=top_k,
                                         computation_method=computation_method)
        ...
        if True:
            ...
            logger.info(
                f'exp(H(X|Y)) = {torch.exp(entropy_joint - entropy_Y).item()}, exp(I(X; Y)) = {torch.exp(entropy_X + entropy_Y - entropy_joint).item()}')
    ...
    def mutual_information_order_2(self, X, Y, order, compute_kernel=True, entropy='shannon'):
        i_xy = self.compute_entropy(X, order=order, compute_kernel=False) + self.compute_entropy(Y, order=order,
                                                                                                 compute_kernel=False) - self.compute_joint_entropy(
            X, Y, order=order, compute_kernel=False)
